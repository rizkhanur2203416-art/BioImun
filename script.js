...
/* ── 5b. JAWABAN LKPD PER TAHAP (dengan konten lengkap) ─────────
   tahap    : 1-5 (1-indexed)
...
function syncAngket(answers, mode) {
  const u = getSyncUser();
  sendToSheet('angket', {
    username: u.username,
    nama    : u.nama,
    answers,                       // array 15 nilai (1-5)
    mode    : mode || 'pre',       // 'pre' atau 'post'
  });
}
...
/* ── Kirim antrean offline saat halaman siap ── */
document.addEventListener('DOMContentLoaded', () => {
  flushSyncQueue();
  console.log('[BioImun Sync] ✅ Modul sinkronisasi aktif.');
});
...
function saveProgress() {
  ...
  // Sync ke Google Sheets — throttle 10 detik agar tidak spam
...
function submitLKPD(idx) {
  ...
  // ── SYNC LKPD ke Google Sheets (jawaban lengkap per tahap) ──
  const tahap = idx + 1;
  if (typeof syncLKPDTahap === 'function') {
    let jawaban = {};
    if (idx === 0) {
      jawaban = { rumusan_masalah: (document.getElementById('rm-text')?.value || '').trim() };
    } else if (idx === 1) {
      jawaban = { rencana_belajar: (document.getElementById('org-text')?.value || '').trim() };
    } else if (idx === 2) {
      jawaban = { hasil_analisis: (document.getElementById('inv-text')?.value || '').trim() };
    } else if (idx === 3) {
      jawaban = { deskripsi_infografis: (document.getElementById('pres-text')?.value || '').trim() };
    } else if (idx === 4) {
      jawaban = {
    ...
    syncLKPDTahap(tahap, window.currentLKPDGroup ?? 1, jawaban);
  } else if (typeof syncLKPD === 'function') {
    syncLKPD(1, idx);
  }

  showToast('✅ Tahap ' + tahap + ' berhasil diselesaikan! Tahap berikutnya terbuka.', 'success');
...
function checkAndSubmitLKPD(idx, textareaId) {
  ...
  // Tahap 5: validasi juga sol-text dan kes-text
  if (idx === 4) {
    const solEl = document.getElementById('sol-text');
    const kesEl = document.getElementById('kes-text');
    if (solEl && solEl.value.trim().length < 20) {
      showToast('⚠️ Isi kolom "Solusi" terlebih dahulu (minimal 20 karakter)!', 'error');
      solEl.focus();
      return;
    }
    if (kesEl && kesEl.value.trim().length < 20) {
      showToast('⚠️ Isi kolom "Kesimpulan" terlebih dahulu (minimal 20 karakter)!', 'error');
      kesEl.focus();
      return;
    }
  }
...
function showDrillResult() {
  ...
  updateGlobalProgress();
  renderBadges();
  // ── SYNC drill ke Google Sheets (dengan detail per soal) ──
  if (typeof syncDrill === 'function') {
    const letters = ['A','B','C','D'];
    const detail = drillQuestions.map((q, i) => ({
      nomor        : i + 1,
      soal         : q.q,
      jawaban_siswa: drillAnswers[i] >= 0 ? letters[drillAnswers[i]] : 'Tidak dijawab',
      jawaban_benar: letters[q.ans],
      benar        : drillAnswers[i] === q.ans,
    }));
    syncDrill(drillNum, score, detail);
  }
}
